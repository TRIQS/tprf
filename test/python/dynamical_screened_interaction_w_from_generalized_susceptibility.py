...
def test_dynamical_screening_functions_single_orbital():
    ...
    for f in fmesh:
        ...
        Wr_fk_ref.data[fi,:] = V_k.data[:] + V_k.data[:] * PI_fk.data[fi,:] * V_k.data[:]
    
    
    print('--> screened_interaction_W (Matsubara, static)')
    Wr_wk_1 = dynamical_screened_interaction_W_from_generalized_susceptibility(PI_wk, V_k)
    ...
    Wr_fk_2 = dynamical_screened_interaction_W_from_generalized_susceptibility(PI_fk, V_fk)
