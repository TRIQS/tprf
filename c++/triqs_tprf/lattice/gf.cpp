...
#pragma omp parallel for
  for (unsigned int idx = 0; idx < arr.size(); idx++) {
    auto &[w, k] = arr[idx];
    g0_wk[w, k] = nda::linalg::inv((w + mu)*I - e_k[k]);      
  }
...
#pragma omp parallel for
  for (int idx = 0; idx < arr.size(); idx++) {
    auto &[f, k] = arr[idx];
    g0_fk[f, k]  = nda::linalg::inv((f + idelta + mu) * I - e_k[k]);
  }
...
#pragma omp parallel for
  for (unsigned int idx = 0; idx < arr.size(); idx++) {
    ...
    g_wk[w, k] = nda::linalg::inv((w + idelta + mu)*I - e_k[k] - sigmaterm);
  }
...
#pragma omp parallel for
    for (unsigned int widx = 0; widx < freqmesh.size(); widx++) {
      ...
      g_w[w] += nda::linalg::inv((w + idelta + mu)*I - e_k[k] - sigmaterm);
    }
  }
