...
template<SusceptibilityType susType, typename chi_t, typename v_t>
auto screened_interaction_from_generic_susceptibility(chi_t &chi, v_t &V) {

  auto &[freqmesh, kmesh] = chi.mesh();

  auto &vkmesh = [&V]() -> auto & {
    if constexpr (v_t::arity == 1) return V.mesh();
    else return std::get<1>(V.mesh());
  }();
...
#pragma omp parallel for
  for (unsigned int idx = 0; idx < arr.size(); idx++) {
    ...
    if constexpr(v_t::arity == 1) V_arr = V[k]; 
    else V_arr = V[w, k]; 
