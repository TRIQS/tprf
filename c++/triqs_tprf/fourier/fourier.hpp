...
// FIXME : DOC
inline gf_mesh<imtime> make_adjoint_mesh(gf_mesh<imfreq> const &m,
                                         int n_tau = -1) {
  if (n_tau == -1)
    n_tau = 2 * (m.last_index() + 1) + 1;
...
// real
  /*
gf_vec_t<refreq> _fourier_impl(gf_mesh<refreq> const &w_mesh,
                               gf_vec_cvt<retime> gt,
                               array_const_view<dcomplex, 2> mom_12 = {});
gf_vec_t<retime> _fourier_impl(gf_mesh<retime> const &t_mesh,
                               gf_vec_cvt<refreq> gw,
                               array_const_view<dcomplex, 2> mom_12 = {});
  */
...
void _fourier(gf_const_view<V1, T> gin, gf_view<V2, T> gout,
              OptArgs const &... opt_args) {

  // gf_mesh<V2> out_mesh = std::get<N>(gout.mesh());
  auto const &out_mesh = std::get<N>(gout.mesh()); // FIXME singlevar??

  auto gout_flatten = _fourier_impl(out_mesh, flatten_gf_2d<N>(gin),
                                    flatten_2d(opt_args, N)...);
  auto _ = ellipsis();
  if constexpr (gin.data_rank == 1)
    gout.data() =
        gout_flatten.data()(_, 0); // gout is scalar, gout_flatten vectorial
  else {
...
// implementation
template <int N, typename V1, typename V2, typename T, typename... OptArgs>
void _fourier_with_plan(gf_const_view<V1, T> gin, gf_view<V2, T> gout,
                        fourier_plan &p, OptArgs const &... opt_args) {

  // gf_mesh<V2> out_mesh = std::get<N>(gout.mesh());
  auto const &out_mesh = std::get<N>(gout.mesh()); // FIXME singlevar??

  auto gout_flatten = _fourier_impl(out_mesh, flatten_gf_2d<N>(gin), p,
                                    flatten_2d(opt_args, N)...);
  auto _ = ellipsis();
  if constexpr (gin.data_rank == 1)
    gout.data() =
...
// implementation
template <int N, typename V1, typename V2, typename T, typename... OptArgs>
fourier_plan _fourier_plan(gf_const_view<V1, T> gin, gf_view<V2, T> gout,
                           OptArgs const &... opt_args) {
  auto const &out_mesh = std::get<N>(gout.mesh());
  auto p = _fourier_plan(out_mesh, flatten_gf_2d<N>(gin),
                         flatten_2d(opt_args, N)...);
  return std::move(p);
}
...
auto make_gf_from_fourier(gf_const_view<V1, T> gin, gf_mesh<V2> const &mesh,
                          OptArgs const &... opt_args) {
  static_assert(get_n_variables<V2>::value == 1,
...
template <int N = 0, typename T>
gf<imtime, T> make_gf_from_fourier(gf_const_view<imfreq, T> gin,
                                   int n_iw = -1) {
  return make_gf_from_fourier(gin, make_adjoint_mesh(gin.mesh(), n_iw));
}

template <int N = 0, typename T>
gf<imfreq, T> make_gf_from_fourier(gf_const_view<imtime, T> gin,
                                   int n_tau = -1) {
  ...
  */
  ...
  /*
template <int N = 0, typename T>
gf<retime, T> make_gf_from_fourier(gf_const_view<refreq, T> gin) {
  return make_gf_from_fourier(gin, make_adjoint_mesh(gin.mesh()));
}

template <int N = 0, typename T>
gf<refreq, T> make_gf_from_fourier(gf_const_view<retime, T> gin) {
  return make_gf_from_fourier(gin, make_adjoint_mesh(gin.mesh()));
}
  */
...
template <int N, typename G> auto operator&(G &&gin, _fou_wk<N> const &) {
  return make_gf_from_fourier<N>(gin,
                                 make_adjoint_mesh(std::get<N>(gin.mesh())));
}
...
template <int N1, int N2, int... Ns, typename V, typename T>
auto make_gf_from_fourier(gf_const_view<V, T> gin) {
  // int rank = get_n_variables<V>::value;
  // static_assert(N1 < rank && N2 < rank , "Mesh position out of bounds");

  auto g1 = make_gf_from_fourier<N1>(
      gin(), make_adjoint_mesh(std::get<N1>(gin.mesh())));
  auto g2 = make_gf_from_fourier<N2>(
      g1(), make_adjoint_mesh(std::get<N2>(g1.mesh())));

  if constexpr (sizeof...(Ns) == 1) {
    auto gres = make_gf_from_fourier<Ns...>(
        g2, make_adjoint_mesh(std::get<Ns...>(g2.mesh())));
...
auto make_gf_from_fourier(block_gf_const_view<V, T> gin, Args &&... args) {
  auto l = [&](gf_const_view<V, T> g_bl) {
    return make_gf_from_fourier<N, Ns...>(make_const_view(g_bl),
...
template <int N = 0, int... Ns, typename V, typename T, typename... Args>
auto make_gf_from_fourier(gf<V, T> const &gin, Args &&... args) {
  return make_gf_from_fourier<N, Ns...>(make_const_view(gin),
                                        std::forward<Args>(args)...);
}

template <int N = 0, int... Ns, typename V, typename T, typename... Args>
auto make_gf_from_fourier(block_gf_view<V, T> gin, Args &&... args) {
  return make_gf_from_fourier<N, Ns...>(make_const_view(gin),
                                        std::forward<Args>(args)...);
}

template <int N = 0, int... Ns, typename V, typename T, typename... Args>
auto make_gf_from_fourier(block_gf<V, T> const &gin, Args &&... args) {
  return make_gf_from_fourier<N, Ns...>(make_const_view(gin),
                                        std::forward<Args>(args)...);
}

template <int N = 0, int... Ns, typename V, typename T, typename... Args>
auto make_gf_from_fourier(block2_gf_view<V, T> gin, Args &&... args) {
  return make_gf_from_fourier<N, Ns...>(make_const_view(gin),
                                        std::forward<Args>(args)...);
}

template <int N = 0, int... Ns, typename V, typename T, typename... Args>
auto make_gf_from_fourier(block2_gf<V, T> const &gin, Args &&... args) {
  return make_gf_from_fourier<N, Ns...>(make_const_view(gin),
                                        std::forward<Args>(args)...);
}
...
_fourier_lazy<N, typename G::const_view_type, Args...>
fourier(G const &g, Args &&... args) {
  return {g(), {std::forward<Args>(args)...}};
}

// FIXME: add gx = fourier(gy) for gx a gf (not a view), adding a
// triqs_gf_assign_delegation and using make_gf_from_fourier

// realize the call for gx = fourier(gy);
template <int N, typename V1, typename T1, typename V2, typename T2,
          typename... Args>
void triqs_gf_view_assign_delegation(
    gf_view<V1, T1> lhs_g,
    _fourier_lazy<N, gf_const_view<V2, T2>, Args...> const &rhs) {
  static_assert(
      std::is_same_v<T1, T2>,
  ...
  if constexpr (get_n_variables<V1>::value == 1) // === single mesh
    ...
    static_assert(std::is_same_v<typename gf_mesh<V1>::m_tuple_t, mesh_res_t>,
                  "Meshes in assignment don't match");
  }

  // check the size of the "inactive" dimensions

  std::apply([&](auto &&... u) { _fourier<N>(rhs.g, lhs_g, u...); },
